...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "rachelg") // Pass request body from file
        ...
        response.then().body("username", equalTo("rachelg"));
        response.then().body("firstName", equalTo("rachel"));
        response.then().body("lastName", equalTo("green"));
        response.then().body("email", equalTo("rachelg@mail.com"));
        response.then().body("password", equalTo("rach@123"));
    ...
    public void deleteUser() throws IOException {
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "rachelg") // Add path parameter
                        .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("rachelg"));
    }
}